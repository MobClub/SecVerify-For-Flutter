...
- (void)_updatePrivacyCheckBoxConstraintsAt:(UIView *)superView
    ...
    BOOL (^layoutIsValidBool) (NSString *) = ^(NSString *key) {
        BOOL isContains = [[layouts allKeys] containsObject:key];
        BOOL isValid = [[layouts objectForKey:key] respondsToSelector:@selector(boolValue)];
        BOOL result = isContains && isValid;
        return result;
    };
    ...
    //相对授权页
    if (layoutIsValidBool(@"layoutToSuperView")) {
    ...
    if (layoutIsValid(@"layoutRightSpaceToPrivacyLeft")) {
    ...
    if (layoutIsValid(@"layoutLeftSpaceToPrivacyRight")) {
